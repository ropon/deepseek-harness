...
The packaging script creates a production deployment, verifies that every required Harness peer provider is resolvable, uses bundled platform prebuilds where available, rebuilds the remaining native dependencies for Electron's ABI, and writes the application under `apps/desktop/out/`. Unpacked builds retain pnpm links as package-relative targets and reject any link that escapes the application; Windows installer builds use pnpm's hoisted production layout so NSIS never follows a cyclic peer-dependency link graph. The packaged backend runs through Electron's run-as-Node mode with the internal-module access required by Cordis HMR, and stores its Harness home below Electron's per-user application-data directory.
...
- The production application is unpacked rather than ASAR-archived so native modules and executable helpers retain ordinary filesystem paths. Normal unpacked builds keep the deployed pnpm graph as package-internal relative symbolic links; Windows installer builds use an equivalent hoisted production graph to avoid cyclic archive traversal.
