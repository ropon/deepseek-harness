...
The package also fills `conversation.details.tool` with `ToolDetails`. The row and details renderers share the same pure card models for `terminal`, `read`, `diff`, `search`, and `web` render intents. Image blocks are removed from flattened text: rows use the conversation owner's message-image renderer, while details declare `tool.details.result.images` for the attachment plugin. Unknown intent tags and malformed wire card data fall back to flattened Tool result text.
