...
本包还通过 `ToolDetails` 填充 `conversation.details.tool`。行 renderer 与详情 renderer 共用同一组面向 `terminal`、`read`、`diff`、`search` 和 `web` render intent 的纯 card model。图片 block 不进入压平文本：工具行使用 conversation 持有方的消息图片 renderer，详情则为附件插件声明 `tool.details.result.images`。未知的 intent 标签和格式错误的 wire card 数据都会回退为压平的工具结果文本。
