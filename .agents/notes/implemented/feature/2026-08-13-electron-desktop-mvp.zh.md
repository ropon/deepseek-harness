...
[`apps/desktop`](../../../../apps/desktop/README.md) 是已交付 Web profile 之上的 Electron 壳。它使用 `web --port 0` 启动已构建的 `dsh` CLI，只接受 CLI 的 loopback 就绪行，并在完整 Loader 配置树结算后加载该源。Electron 负责单一应用实例、后端子进程、合并日志、应用窗口和有界关闭。
...
桌面端持有一层排在用户 Web profile 之后的组合 patch。该 patch 通过绝对 package URL 绑定安装包内置的 `dsh-plugin-clawrouters`，而不修改用户 profile。共享 Models 引导会在这条路由存在时优先选择它，因此全新桌面会询问 `CLAWROUTERS_API_KEY`，原生 DeepSeek 凭据弹窗只作为回退。生图工具结果仍是标准附件 block；通用工具界面会通过会话附件解析器加载并渲染这些 block。
