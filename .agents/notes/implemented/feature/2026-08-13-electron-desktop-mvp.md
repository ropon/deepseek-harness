...
The application remains unpacked instead of using ASAR because Harness dependencies include native modules and executable helpers that require ordinary filesystem paths. Packager retains the production deployment's pnpm graph without dereferencing legitimate peer cycles, then rewrites staging-absolute links to package-internal relative targets and verifies that every link resolves inside the application. Code signing, notarization, and updates remain release-distribution work rather than startup behavior.
