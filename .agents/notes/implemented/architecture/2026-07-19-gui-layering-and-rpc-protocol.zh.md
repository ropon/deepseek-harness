...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - [Electron 桌面 MVP](../feature/2026-08-13-electron-desktop-mvp.md) 通过 loopback 复用 Web 载体；更深层的桌面集成经由 IPC fetch 载体复用同一套 web client 包。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 浏览器 HTTP 与 loopback 桌面 MVP；零 workspace 依赖（注册表经结构注入到达）；集成 IPC 的桌面应用不复用它 |
...
1. **选 fetch 伪造方式**：浏览器同源 HTTP / 进程内 `host.handler.fetch` 注入 / 自写传输切面子类（如 Electron IPC，见下文「子类表」）。
...
| IPC 桥子类（已预留；Electron MVP 使用 loopback） | 更深层的 Electron 集成 | IPC 序列化往返 | 只需换 doFetch，约定/基类保持不变 |
