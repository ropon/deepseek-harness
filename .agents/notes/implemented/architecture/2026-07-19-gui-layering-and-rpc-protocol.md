...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - The [Electron desktop MVP](../feature/2026-08-13-electron-desktop-mvp.md) reuses the Web carrier on loopback; a deeper desktop integration reuses the same client packages over an IPC fetch carrier.
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser HTTP plus the loopback desktop MVP; zero workspace dependencies (the registry arrives by structural injection); an IPC-integrated desktop app does not reuse it |
...
1. **Pick a fetch impersonation**: browser same-origin HTTP / in-process `host.handler.fetch` injection / your own transport-aspect subclass (e.g. Electron IPC, see the "Subclass table" below).
...
| IPC bridge subclass (reserved; the Electron MVP uses loopback) | a deeper Electron integration | IPC serialization round trip | swaps only doFetch; contract and base class stay unchanged |
